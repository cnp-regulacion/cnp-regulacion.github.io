...
function dibujarFlechas(matriz) {
        ...
        for (var j = 0; j < matriz.nodos[i].length; j++) {
            if (matriz.nodos[i][j] != undefined) {
                for (var k = 0; k < matriz.nodos[i][j].flechas.length; k++) {
                    dibujarFlecha(matriz, matriz.nodos[i][j], j, i, matriz.nodos[i][j].flechas[k])
                }
            }
        }
    }
    flechas.selectAll("flecha")
        ...
        .attr('d', function (d) {
            return " M " + (50 + d.x1 * (boxWidth + separation)) + ", " + (100 + d.y1 * (boxHeight + separation)) + " h " + (boxWidth / 2 + separation / 2) + " V " + (100 + d.y2 * (boxHeight + separation)) + "H " + (50 + d.x2 * (boxWidth + separation))
        })
...
function dibujarFlecha(matriz, desde, x1, y1, hacia) {
    var n2
    for (var i = 0; i < matriz.nodos.length; i++) {
        for (var j = 0; j < matriz.nodos[i].length; j++) {
