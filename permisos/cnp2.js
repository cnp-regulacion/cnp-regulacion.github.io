...
cnp2.Chart = function () {
    return {
        ...
        rScale: d3.scale.linear().domain([0, 542]).range([20, 110]),
        radiusScale: null,
        changeScale: d3.scale.linear().domain([-0.28, 0.28]).range([620, 180]).clamp(true),
        sizeScale: d3.scale.linear().domain([0, 542]).range([0, 1]),
        groupScale: {},

        //data settings
        currentYearDataColumn: 'budget_2013',
        data: cnp2.budget_array_data,
        ...
        init: function () {
            ...
            cnp2.category_data.sort(function (a, b) {
            ...
            currentX = 0,
                currentY = 0;
            for (var i = 0; i < cnp2.category_data.length; i++) {
                var t = 0,
                    w,
                    numInRow = -1,
                    positionInRow = -1,
                    currentRow = -1,
                    cat = cnp2.category_data[i]['label'];
                ...
                if (numInRow === -1) {
                    numInRow = cnp2.category_data.length - d3.sum(columns);
                    currentRow = columns.length;
                    positionInRow = i - d3.sum(columns)
                }
                cnp2.category_data[i].row = currentRow;
                cnp2.category_data[i].column = positionInRow;
            ...
            this.svg = d3.select("#cnp2-chartCanvas").append("svg:svg")
                .attr("width", this.width);


            var departmentOverlay = $j("#cnp2-departmentOverlay");

            for (var i = 0; i < cnp2.category_data.length; i++) {
                var cat = cnp2.category_data[i]['label'];
                var catLabel = cnp2.category_data[i]['short_label'];
                var catTot = cnp2.category_data[i]['total'];
                var catWidth = this.categoryPositionLookup[cat].w;
                var catYOffset = this.categoryPositionLookup[cat].offsetY;
                var catNode;
                if (catLabel === "Other") {
                    catNode = $j("<div class='cnp2-departmentAnnotation cnp2-row" + cnp2.category_data[i]['row'] + "'><p class='department'>" + catLabel + "</p></div>")

                } else {
                    catNode = $j("<div class='cnp2-departmentAnnotation cnp2-row" + cnp2.category_data[i]['row'] + "'><p class='department'>" + catLabel + "</p><p class='total'>" + catTot + "</p></div>")
...
//Draw arc paths
            ...
            this.circle.enter().append("svg:circle")
                ...
                .on("mouseover", function (d, i) {
                    ...
                    d3.select("#cnp2-tooltip").style('top', ypos + "px").style('left', xpos + "px").style('display', 'block');
                    d3.select("#cnp2-tooltip .cnp2-name").html(d.name);
                    d3.select("#cnp2-tooltip .cnp2-value").html(d.value + " Permisos")

                })
                .on("mouseout", function (d, i) {
                    d3.select(this).style("stroke-width", 0);
                    d3.select("#cnp2-tooltip").style('display', 'none')
                });
...
cnp2.ChooseList = function (node, changeCallback) {
...
cnp2.ChooseList.prototype.onClickHandler = function (evt) {
    evt.preventDefault();
    this.selectByElement(evt.currentTarget);
};


cnp2.ChooseList.prototype.selectByIndex = function (i) {
    this.selectByElement(this.elements[i])
};


cnp2.ChooseList.prototype.selectByElement = function (el) {
...
cnp2.ready = function () {
    ...
    cnp2.mainNav = new cnp2.ChooseList($j(".cnp2-navigation"), onMainChange);
    ...
    function onMainChange(evt) {
        ...
        if (tabIndex === 0) {
            cnp2.c.totalLayout();
            this.currentOverlay = $j("#cnp2-totalOverlay");
            this.currentOverlay.delay(300).fadeIn(500);
            $j("#cnp2-chartFrame").css({
                'height': 850
            });
        } else if (tabIndex === 1) {
            cnp2.c.departmentLayout();
            this.currentOverlay = $j("#cnp2-departmentOverlay");
            this.currentOverlay.delay(300).fadeIn(500);
            $j("#cnp2-chartFrame").css({
                'height': 850
            });
...
if (!!document.createElementNS && !!document.createElementNS('http://www.w3.org/2000/svg', "svg").createSVGRect) {
    $j(document).ready($j.proxy(cnp2.ready, this));
} else {
    $j("#cnp2-chartFrame").hide();
}